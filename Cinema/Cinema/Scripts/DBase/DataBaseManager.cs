        ...
        public static Status SingIn(string userLogin, string userPassword)
        {
            ...
            try
            {
                if (DataBaseConnection.ConnectionOpen())
                {
                    ...
                    while (dataReader.Read() && result.Equals(Status.NotFound))
                    {
                        Account.Input(id: uint.Parse(dataReader[0].ToString()), userName: dataReader[1].ToString(), login: dataReader[3].ToString(), password: dataReader[4].ToString(), isActive: bool.Parse(dataReader[5].ToString()));
